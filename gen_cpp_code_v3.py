...
for layer_type, cnt in counter.items():
    if layer_type == 0 and cnt > 0:
        ...
        def_cnn_layer += '            int_conv2d_layer<bit_64, bit_%d, 64, %d, %d, %d>(in_img, fb_tmp, conv0W, b0_BNFb);\n' % (max_bconv_width, int(n_ou_fmaps[0]), int(infmap_siz[0]), int(infmap_siz[0]))
        def_cnn_layer += '            break;\n'
...
for i in range(len(initial_options)):
    ...
    if initial_options[i] == 1:
        ...
        bconv_bias_select += '                case %d:\n' % (conv_idx)
        bconv_bias_select += '                    bias = b%d_BNFb[ofeat];\n' % (conv_idx)
        bconv_bias_select += '                    break;\n'

        conv_idx += 1
bconv_reg_select += '        default:\n'
bconv_reg_select += '            break;\n'
bconv_weight_select += '                    default:\n'
bconv_weight_select += '                        break;\n'
bconv_bias_select += '                default:\n'
bconv_bias_select += '                    break;\n'
...
for i in range(len(initial_options)):
    if initial_options[i] == 0 or initial_options[i] == 1:
        ...
        read_weight_mem += '    if ((fp = fopen("conv%dW.txt", "r")) == NULL) {\n' % conv_idx
        read_weight_mem += '        fprintf(stderr, "CANNOT OPEN\\n");\n'
        read_weight_mem += '    }\n'
        ...
        read_weight_mem += '                    if (fgets(line, 256, fp) == NULL) {\n'
        read_weight_mem += '                        fprintf(stderr, "EMPTY FILE READ\\n");\n'
        read_weight_mem += '                    }\n'
        read_weight_mem += '                    sscanf(line, "%d", &d_value);\n'
        ...
        read_bias_mem += '    if ((fp = fopen("b%d_BNFb.txt", "r")) == NULL) {\n' % bn_idx
        read_bias_mem += '        fprintf(stderr, "CANNOT OPEN\\n");\n'
        read_bias_mem += '    }\n'
        read_bias_mem += '    offset = %d;\n' % offset_bias
        read_bias_mem += '    for (of = 0; of < %d; of++) {\n' % int(n_ou_fmaps[i])
        read_bias_mem += '        if (fgets(line, 256, fp) == NULL) {\n'
        read_bias_mem += '            fprintf(stderr, "EMPTY FILE READ\\n");\n'
        read_bias_mem += '        }\n'
        ...
        read_weight_mem += '    if ((fp = fopen("fc%dW.txt", "r")) == NULL) {\n' % dense_idx
        read_weight_mem += '        fprintf(stderr, "CANNOT OPEN\\n");\n'
        read_weight_mem += '    }\n'
        read_weight_mem += '    offset = %d;\n' % offset_weight
        read_weight_mem += '    for (of = 0; of < %d; of++) {\n' % int(n_ou_fmaps[i])
